...
class PongGame {
    ...
    loop(now) {
        ...
        // Cap dt to prevent massive tunneling if tab was backgrounded for minutes
        // We'll simulate at most 0.1s steps to catch up logic, but no more than 1s total
        let updateDt = dt;
        if (updateDt > 1.0) updateDt = 1.0;

        // Physics Update (Sub-stepping for stability)
        const stepSize = 0.016; // ~60fps step
        while (updateDt > 0) {
    ...
    updatePhysics(dt) {
        ...
        if (this.state.ball.x < PADDLE_WIDTH + 5 &&
            ...
            this.state.ball.y + BALL_SIZE >= this.state.paddle1Y &&
            this.state.ball.y <= this.state.paddle1Y + PADDLE_HEIGHT) {
            // Determine bounce direction (+ speed up slightly)
            this.state.ball.dx = Math.abs(this.state.ball.dx) * 1.05;
            if (this.state.ball.dx > 600) this.state.ball.dx = 600; // Cap speed
        ...
        if (this.state.ball.x + BALL_SIZE > CANVAS_WIDTH - PADDLE_WIDTH - 5 &&
            ...
            this.state.ball.y + BALL_SIZE >= this.state.paddle2Y &&
            this.state.ball.y <= this.state.paddle2Y + PADDLE_HEIGHT) {
            this.state.ball.dx = -Math.abs(this.state.ball.dx) * 1.05;
